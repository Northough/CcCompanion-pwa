  ...
  return (
    <div style={{ margin: '4px 0 14px' }}>
      <div className="codeblock-head">
        ...
        <button onClick={() => navigator.clipboard?.writeText(code)} style={{ background: 'transparent', border: 'none', color: '#9fb0bc', display: 'inline-flex', alignItems: 'center', gap: 4, fontFamily: 'inherit', fontSize: 'inherit', cursor: 'pointer' }}>
...
export function PermissionRequest({ tool, args, description, onApprove, onDeny }: { tool: string; args?: Record<string, unknown>; description?: string; onApprove: () => void; onDeny: () => void }) {
  return (
    <div style={{ margin: '4px 0 14px', background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 16, padding: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>Permission required</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button onClick={onDeny} style={{ flex: 1, padding: '10px 0', borderRadius: 999, background: 'transparent', border: '1px solid var(--line-2)', color: 'var(--muted)', fontSize: 13.5, fontWeight: 600 }}>Deny</button>
